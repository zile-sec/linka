...
class UserSignup(BaseModel):
    email: EmailStr
    password: str
    role: str  # "customer", "retailer", or "business"
...
def signup(user: UserSignup):
    valid_roles = ["customer", "retailer", "business"]
    ...
    if user.role == "business":
        valid_categories = ["cloths", "service provider", "tech", "food", "delivery"]
        if not user.category or user.category not in valid_categories:
            raise HTTPException(status_code=400, detail=f"Business role requires a valid category: {valid_categories}")
    ...
    try:
        ...
        data = {
            ...
            "data": {"role": user.role, "category": user.category}
        }
